...
############################################
        ...
        # 必要なら絶対パス化
        if not os.path.isabs(live2d_model_dir):
            live2d_model_dir = os.path.abspath(live2d_model_dir)

        # 以降は live2d_model_dir を使う        
        # dict型の場合
        #if isinstance(frontend_config, dict):
        ...
        async def serve_live2d_file(file_path: str):
            ...
            try:
                ...
                full_path = os.path.abspath(os.path.join(live2d_model_dir, file_path))
                base_path = os.path.abspath(live2d_model_dir)
